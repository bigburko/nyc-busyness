...
interface MyDrawerProps {
  ...
  // ✅ FIX: Removed the zIndex prop as it's not a valid prop for <Drawer>
}

// ... ALL_AVAILABLE_LAYERS constant is correct ...
...
export default function MyDrawer({ isOpen, onClose, onSearchSubmit }: MyDrawerProps) {
  // ... (all your existing hooks and handlers are correct) ...
  ...
  const handleWeightChangeEnd = useCallback((id: string, value: number) => { updateWeight(id, value); }, [updateWeight]);
  const handleAddWeight = useCallback((layer: Layer) => { const newWeight: Weighting = { ...layer, value: 15 }; const updatedWeights = [...activeWeights, newWeight]; setFilters({ weights: updatedWeights }); }, [activeWeights, setFilters]);
  const handleRemoveWeight = useCallback((id: string) => { const updatedWeights = activeWeights.filter((w: Weighting) => w.id !== id); setFilters({ weights: updatedWeights }); }, [activeWeights, setFilters]);
  ...
  const handleSubmit = () => { if (!selectedGenders.length) { alert('Please select at least one gender.'); return; } const currentState = useFilterStore.getState(); onSearchSubmit(currentState); onClose(); };


  return (
    <main>
      {/* ✅ FIX: Removed zIndex prop from here */}
      <Drawer isOpen={isOpen} onClose={onClose} finalFocusRef={btnRef as unknown as React.RefObject<FocusableElement>} placement="left" size="sm">
        {/* The rest of your Drawer component remains exactly the same */}
        <DrawerOverlay />
        <DrawerContent bg="#FFDED8" display="flex" flexDirection="column" h="100%">
          ...
          <DrawerBody ref={drawerBodyRef} overflowY="auto" css={{ '&::-webkit-scrollbar': { width: '8px' }, '&::-webkit-scrollbar-track': { background: '#f1f1f1', borderRadius: '4px' }, '&::-webkit-scrollbar-thumb': { background: '#888', borderRadius: '4px', '&:hover': { background: '#555' } } }}>
            <Flex direction="column" gap={4} pb={8}>
              <Box bg="white" borderRadius="lg" p={4} boxShadow="sm"><MyRangeSlider heading="Rent (PSF)" toolTipText="Target Average Rent cost per Square foot in $USD" defaultRange={rentRange} onChangeEnd={handleRentRangeChange}/></Box>
              <Box bg="white" borderRadius="lg" p={3} boxShadow="sm"><CollapsibleSection title="Demographic Fit Filters" tooltip="Customize your target audience by age, income, gender, and race"><Flex direction="column" gap={4}><MyAgeSlider value={ageRange} onChangeEnd={handleAgeRangeChange} /><MyIncomeSlider value={incomeRange} onChangeEnd={handleIncomeRangeChange} /><GenderSelect value={selectedGenders} onChange={handleGenderChange} /><Box mt={2} ref={ethnicityRef} borderRadius="md" minHeight="60px"><HierarchicalMultiSelect data={ethnicityData} label="Select Ethnicities" onChange={handleEthnicityChange} autoFocus={false} onMenuOpenChange={setMenuIsOpen} controlledInput={dropdownInput} setControlledInput={setDropdownInput} externalSelectedValues={selectedEthnicities} externalExpandedGroups={expandedGroups} setExternalExpandedGroups={setExpandedGroups} setMenuIsOpenExternal={setMenuIsOpen} selectWrapperRef={selectWrapperRef}/></Box>{_menuIsOpen && <Box h="280px" />}</Flex></CollapsibleSection></Box>
              <Box bg="white" borderRadius="lg" p={3} boxShadow="sm"><CollapsibleSection title="Resilience Score Weighting" tooltip="Adjust how different factors contribute to the overall score"><WeightingPanel activeWeights={activeWeights} inactiveLayers={ALL_AVAILABLE_LAYERS.filter((layer: Layer) => !activeWeights.some((active: Weighting) => active.id === layer.id))} onSliderChangeEnd={handleWeightChangeEnd} onRemove={handleRemoveWeight} onAdd={handleAddWeight}/><Flex justify="center" w="100%" mt={4}><Button size="sm" bg="black" color="white" _hover={{ bg: 'black' }} _active={{ bg: 'black' }} borderRadius="md" px={4} py={2} onClick={() => setIsResetDialogOpen(true)}>Reset Weights to Default</Button></Flex></CollapsibleSection></Box>
            </Flex>
          </DrawerBody>
      ...
      <AlertDialog isOpen={isResetDialogOpen} leastDestructiveRef={cancelRef as unknown as React.RefObject<FocusableElement>} onClose={() => setIsResetDialogOpen(false)}>
        <AlertDialogOverlay><AlertDialogContent><AlertDialogHeader fontSize="lg" fontWeight="bold">Reset Weightings</AlertDialogHeader><AlertDialogBody>Are you sure you want to reset all weightings to their default values? This cannot be undone.</AlertDialogBody><AlertDialogFooter><CancelResetButton ref={cancelRef} onClick={() => setIsResetDialogOpen(false)} /><Button colorScheme="red" onClick={() => { setFilters({ weights: INITIAL_WEIGHTS }); setIsResetDialogOpen(false); }} ml={3}>Reset</Button></AlertDialogFooter></AlertDialogContent></AlertDialogOverlay>
      </AlertDialog>
    </main>
  );
}